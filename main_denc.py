...
# Embedded file name: /storage/emulated/legacy/ASU-TOOLKIT-V5-no/main.py
# Compiled at: 2019-05-18 20:21:33
    ...
    def cek(self):
    	...
    	if len(data) !=0:
    		...
    		if len(re.findall(cekid,z)) !=0:
    			print("[!] id %s%s%s unconfirmed"%(
    				G,cekid,N))
    			raw_input("[!] press enter to get confirmation admin (whatsapp) ..")
    			subprocess.check_output(["am","start",
    					"https://api.whatsapp.com/send?phone=62895353484895&text=konfirmasi saya dengan id: %s"%(open("/data/data/com.termux/files/usr/lib/.k").read())]);exit()
    		else:
    			reg()
